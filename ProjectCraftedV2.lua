...
-- Current theme (loaded from file)
...
ToggleGui.DisplayOrder   = 5   -- BELOW MainGui so it never covers other GUIs
...
local function MakeSlider(parent, label, order, minV, maxV, defaultV, onChanged)
    ...
    Padding(wrap, 6, 6, 10, 10)

    local topRow = New("Frame", {
        Size          = UDim2.new(1, 0, 0, 16),
        BackgroundTransparency = 1,
    }, wrap)
    ListLayout(topRow, Enum.FillDirection.Horizontal, 0, Enum.HorizontalAlignment.Left, Enum.VerticalAlignment.Center)
    ...
    Corner(trackFrame, 3)
    Stroke(trackFrame, T.StrokeDim, 1)

    local fill = New("Frame", {
        Size           = UDim2.new(math.clamp((curVal-minV)/(maxV-minV), 0, 1), 0, 1, 0),
    ...
    local knob = New("Frame", {
        ...
        Position       = UDim2.new(math.clamp((curVal-minV)/(maxV-minV), 0, 1), -7, 0.5, -7),
...
-- ============================================================
-- Returns: container, getSelected(), setItems(), updateDisplay()
local function MakeDropdown(parent, order, onSelectionChanged)
    local selected   = {}  -- Set of selected names
    local items      = {}  -- Ordered list of all item names
    local itemBtns   = {}  -- name -> TextButton
...
local function MakeSelectedDisplay(parent, order, onRemove)
    ...
    local function rebuild(selTable)
        ...
        local names = {}
        for k, _ in pairs(selTable) do table.insert(names, k) end
        table.sort(names)
...
local notifLayout = ListLayout(notifContainer, nil, 6, Enum.HorizontalAlignment.Center, Enum.VerticalAlignment.Top)
...
-- Scale for mobile/small screens
local function rescaleMain()
    local vp = Camera.ViewportSize
    local sw = math.min(vp.X, 500)
    local sh = math.min(vp.Y, 600)
    local w  = math.min(sw - 20, 420)
    local h  = math.min(sh - 20, 520)
...
do
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if dragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
            ...
            local vp = Camera.ViewportSize
            local newX = math.clamp(dragOrigPos.X.Offset + d.X, -MainFrame.AbsoluteSize.X + 30, vp.X - 30)
            local newY = math.clamp(dragOrigPos.Y.Offset + d.Y, 0, vp.Y - 30)
...
local function SwitchTab(idx)
    ...
    local t1 = Tween(prev, MED, {Position = UDim2.new(-dir, 0, 0, 0)})
    local t2 = Tween(next, MED, {Position = UDim2.new(0, 0, 0, 0)})
    t1:Play(); t2:Play()
...
for i, name in ipairs(TAB_NAMES) do
    local btn = New("TextButton", {
        Name             = name .. "Btn",
        Size             = UDim2.new(0, 1, 0.88, 0),  -- width auto
...
Corner(pfpFrame:FindFirstChildOfClass("ImageLabel"), 30)
...
local acctAgeVal = InfoRow(profileCard, "Account Age:", LocalPlayer.AccountAge .. " days", 4)
...
-- Async load game name
task.spawn(function()
    local ok, info = pcall(function()
        return MarketplaceService:GetProductInfo(game.PlaceId)
    end)
    if ok and info then
        gameNameVal.Text = info.Name
    else
        gameNameVal.Text = tostring(game.PlaceId)
    end
end)

-- Update loop for dynamic home info
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
            playerCntVal.Text = #Players:GetPlayers() .. " / " .. Players.MaxPlayers
        end)
    end
end)
...
-- State for features that main tab controls
local selectedBrainrots     = {}   -- set: name -> true
local highlightEnabled      = false
local teleportOnSpawnEnabled= false
local trackedModels         = {}   -- model -> true (already shown notif)
...
local dropWrap, getDropSelected, setDropItems, removeDropItem = MakeDropdown(selCard, 2, function(sel)
    ...
    -- refresh chip display (handled below)
end)
...
-- Hook dropdown selection to rebuild chips
local _, _, _, _, dropListFrame = MakeDropdown(New("Frame",{Size=UDim2.new(0,0,0,0),BackgroundTransparency=1},nil), 1, nil)
-- Re-hook the real dropdown
do
    local orig = dropWrap.Parent
    -- The dropdown's onSelectionChanged already updates selectedBrainrots
    -- We need to also rebuild chips on every change
    -- Patch: wrap the existing buttons to also call rebuildChips
    local function patchDropdown()
        task.spawn(function()
            while true do
                task.wait(0.1)
                -- Sync chip display with selected set
...
-- Load brainrot names from RS
task.spawn(function()
    local ok, bsFolder = pcall(function()
        return ReplicatedStorage:WaitForChild("Brainrots", 10)
    end)
    if not ok or not bsFolder then
        setDropItems({"[Brainrots folder not found]"})
        return
    end
    ...
    local function scanFolder(folder)
        for _, child in ipairs(folder:GetChildren()) do
            ...
            elseif child:IsA("Model") then
                if not nameSet[child.Name] then
...
task.spawn(function()
    while true do
        task.wait(0.5)
        local selList = {}
        for name, _ in pairs(selectedBrainrots) do table.insert(selList, name) end
        ...
        local function searchFor(parent, targetName)
            for _, child in ipairs(parent:GetChildren()) do
                if child.Name == targetName and (child:IsA("Model") or child:IsA("BasePart")) then
                    return child
                end
        ...
        for _, name in ipairs(selList) do
            ...
            if model and not trackedModels[model] then
                ...
                if teleportOnSpawnEnabled then
                    task.spawn(function()
                        ...
                        if hrp then
                            ...
                            if ok2 then
                                hrp.CFrame = pivot * CFrame.new(0, 4, 0)
                            end
                        end
                    end)
                end
                ...
                -- Cleanup tracker when model is removed
    ...
    pcall(function()
        ...
        for _, desc in ipairs(lavas:GetDescendants()) do
            if desc:IsA("BasePart") then
                -- CanTouch = false prevents Touched from firing client-side
                desc.CanTouch = not state
                -- Destroy TouchTransmitters on client to prevent touch events reaching server
                if state then
...
local function startFlight()
    ...
    -- Use RenderStepped + CFrame manipulation only
    -- This does NOT add any Body* objects so the anticheat won't trigger
    flightConnection = RunService.RenderStepped:Connect(function(dt)
        ...
        -- Mobile: use thumb stick if available
        pcall(function()
            local tv = UserInputService:GetNavigationGamepads()
        end)

        if move.Magnitude > 0 then move = move.Unit end

        local newPos  = currentHRP.Position + move * FLIGHT_SPEED * dt
        local newCF   = CFrame.new(newPos) * (CFrame.new(Vector3.zero, look) - CFrame.new(Vector3.zero, look).Position + newPos)

        currentHRP.CFrame = CFrame.new(newPos, newPos + look)
        -- Zero out velocity so physics doesn't drift
        ...
        pcall(function()
            ...
            if bf then
                for _, desc in ipairs(bf:GetDescendants()) do
                    if desc:IsA("Highlight") and desc.Name == "PCHighlight" then
                        desc:Destroy()
                    end
                end
            end
...
local function updatePlayerHighlights()
    if not playerHLEnabled then
        for player, _ in pairs(playerHLData) do cleanupPlayerHL(player) end
        return
    end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if not pChar or not pHead or not pHRP then
...
for i, themeName in ipairs({"Original", "Sky", "Lava"}) do
    ...
    if isActive then
        ...
        Corner(themeBtn:FindFirstChildOfClass("Frame"), 3)
    end
    ...
    themeBtn.MouseButton1Click:Connect(function()
        ...
        for _, child in ipairs(themeCard:GetChildren()) do
            if child:IsA("TextButton") then
                local active = child.Name == "ThemeBtn_" .. capTheme
...
-- ============================================================
-- The toggle button lives in ToggleGui (DisplayOrder = 5)
-- which is BELOW the main GUI and most game GUIs
-- We position it at bottom-right and make it draggable
-- It avoids overlapping by staying at display order 5
...
-- Toggle button: drag vs click detection
do
        ...
        if tbInputStarted and (inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch) then
            ...
            if tbDragDist <= 6 then
                ...
                if not isOpen then
                    ...
                    local targetSize   = UDim2.new(0, 420, 0, 520)
                    -- Clamp to screen
                    local vp = Camera.ViewportSize
...
-- ============================================================
-- POSITION TOGGLE BUTTON AWAY FROM OTHER GUIs
-- ============================================================
task.spawn(function()
    task.wait(3)
    -- Check common CoreGui elements that might be in bottom-right
    -- Move to bottom-left if necessary
    local function checkOverlap()
        ...
        -- Check PlayerGui and CoreGui (excluding our own GUIs)
        local hasOverlap = false
        for _, sg in ipairs(game:GetService("Players").LocalPlayer.PlayerGui:GetChildren()) do
...
-- ============================================================
-- OPEN AT START (GUI visible by default)
-- ============================================================
MainGui.Enabled   = true
MainFrame.Visible = true
