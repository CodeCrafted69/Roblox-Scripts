...
Main = (function()
	...
	Main.InitEnv = function()
		...
		env.decompile = function(scriptPath)
			local okBytecode, bytecode = pcall(getscriptbytecode, scriptPath)

			if not okBytecode then
				return `-- Failed to get script bytecode, error:\n\n--[[\n{bytecode}\n--]]`
			end

			local okRequest, httpResult = pcall(env.request, {
			...
			if httpResult.StatusCode ~= 200 then
				return `-- Failed to decompile with status code: {httpResult.StatusCode}`
			end

			return `-- Decompiled with Medal\n{string.gsub(httpResult.Body, string.char(0x00CD), " ")}`
		end
